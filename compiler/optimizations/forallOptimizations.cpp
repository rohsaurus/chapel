...
//                                                //
////////////////////////////////////////////////////
static bool doesCallHaveNestedCallBase(CallExpr *call);
...
void doPreNormalizeArrayOptimizations() {
  ...
  if (anyAnalysisNeeded) {
    ...
    forv_expanding_Vec(ForallStmt, forall, gForallStmts) {
      // added by tbrolin; do IE opt first, if enabled, since
      // it makes clones of loops which the other opts can then
      // process
      if (fIEOpt) {
...
static void inspectorExecutor(ForallStmt *forall)
{
  ...
  // above with the MAYBE_IE_CANDIDATE primitive.
...
/*
    ...
    A candidate is of the form A[expr(B[expr])], where "expr" must be a binary operation (i.e.,
    operations with two operands) between immediates or variables that are yielded by loops that 
    iterate over arrays/domains. These loops can be enclosed in the forall, or they can enclose the forall itself.
...
static void findCandidatesIE(ForallStmt* forall)
{
  ...
  for_vector(CallExpr, call, callExprs) {
    ...
    if (getCallBase(call)) {
      ...
      if (doesCallHaveNestedCallBase(call)) {
        continue;
      }
      ...
      while (isValid) {
        if ((curr = toCallExpr(curr->parentExpr))) {
          if (curr->isPrimitive(PRIM_MAYBE_IE_CANDIDATE) ||
              curr->isPrimitive(PRIM_MAYBE_AGGR_CANDIDATE) ||
      ...
      while (tmp != NULL && tmp->parentExpr != NULL) {
        if (tmp->isPrimitive(PRIM_MAYBE_IE_CANDIDATE) ||
            tmp->isPrimitive(PRIM_MAYBE_AGGR_CANDIDATE) ||
            tmp->isPrimitive(PRIM_MAYBE_PREFETCH_CANDIDATE)) {

          isValid = false;
          break;
        }
        // Make sure we're not an argument to the prefetch() procedure
                               ...
                               ForallStmt *forall)
{
  ...
  // A[B[i]] cannot be on the LHS of an operation (i.e., a write). We can check for this
  // by seeing if parentCall's parent is an assignment, and if parentCall is
...
static void generateOptimizedLoopsIE(ForallStmt* forall)
{
  ...
  // 5.) Replace the candidate with the MAYBE_IE_CANDIDATE primitive.
  //     Set access type to EXECUTOR_ACCESS; when we clone to create the
  //     inspector loop, we'll go through and change the access type in that
  //     clone to INSPECTOR_ACCESS
...
//##################################################################################
/*
    Creates the MAYBE_IE_CANDIDATE primitive for the IE optimization. The point
                              ...
                              Expr* call)
{
  ...
  CallExpr *repl = new CallExpr(PRIM_MAYBE_IE_CANDIDATE, new_IntSymbol(accessType));
...
static void generateInspectorExecutorLoops(ForallStmt* forall)
{
  ...
  ieHash->addFlag(FLAG_IE_PRIMITIVE_ARG);
  ...
  ieID->addFlag(FLAG_IE_PRIMITIVE_ARG);
  ...
  for_vector(CallExpr, call, callExprs) {
    if (call->isPrimitive(PRIM_MAYBE_IE_CANDIDATE)) {
  ...
  // the MAYBE_IE_CANDIDATE primitive
...
/*
    ...
    primitives -> vector of MAYBE_IE_CANDIDATE primitives in the loop
...
/*
    ...
    be removed (to form the inpector loop) to the vector exprsToRemove. We pass in 
    the MAYBE_IE_CANDIDATE primitive as we do certain things when we detect it.
    ...
    prim -> our primitive MAYBE_IE_CANDIDATE
                               ...
                               std::map<Symbol *, bool> &symbolsToTrack)
{
  ...
  for_alist(node, block->body) {
    ...
    // If node is PRIM_MAYBE_IE_CANDIDATE or is a CallExpr that
    // contains PRIM_MAYBE_IE_CANDIDATE as an argument, replace
    // node with the primitive
    if (CallExpr *call = toCallExpr(node)) {
...
//##################################################################################
/*
    This gets called during prefolding when we come across PRIM_MAYBE_IE_CANDIDATE.
    ...
    As of 4/14/2022, PRIM_MAYBE_IE_CANDIDATE looks like this for A[B[i]]:
    ...
    2.) If step (1) tells us the optimization is invalid, we need to go back
        ...
        We also then need to change the PRIM_MAYBE_IE_CANDIDATE back to A[B[i]]. We
    ...
    3.) If step (1) tells us the optimizaiton is valid, we process the primitive
        ...
        result in either case is that PRIM_MAYBE_IE_CANDIDATE is changed into
    ...
    Arguments:
    call -> The MAYBE_IE_CANDIDATE primitive
...
} /* end of preFoldMaybeIECandidate */
...
/*
    ...
    4.) All loop iterator syms involved in the access must be arrays/domains
...
static bool isIEOptimizationValid(CallExpr *call)
{
  ...
  if (isRecordA) {
    ...
    forv_Vec(CallExpr, c, gCallExprs) {
      if (c->isPrimitive(PRIM_MAYBE_IE_CANDIDATE)) {
...
//##################################################################################
/*
    Given a CallExpr call that is the PRIM_MAYBE_IE_CANDIDATE primitive, where we
    ...
    Note that we track accesses to t across function calls if t is passed into
    a function.
...
/*
    ...
    from preFoldMaybeIECandidate(). The first thing we do is find the enclosing
    forall for call (which is the PRIM_MAYBE_IE_CANDIDATE node).
    ...
    Arguments:
    call -> The MAYBE_IE_CANDIDATE primitive that is part of the inspector loop
...
/*
    ...
    from preFoldMaybeIECandidate(). The first thing we do is find the enclosing
    forall for call (which is the PRIM_MAYBE_IE_CANDIDATE node). From there,
    ...
    Arguments:
    call -> The MAYBE_IE_CANDIDATE primitive that is part of the executor loop
...
//##################################################################################
/*
    Change the MAYBE_IE_CANDIDATE primitive to A[B[i]]. 
...
/*
    ...
    will occur via function calls that accept the symbol as arguments. At this point in post-resolution,
                                          ...
                                          std::map<int, bool> &executorProcessed)
{
  ...
  //     CallExprs, where the CallExpr is the actual write. We also associate
  //     a bool with each CallExpr so we can mark it as a valid or invalid
  //     write.
  //
  ...
  // 5.) We need to do the same types of checks as (4) but for each index iterator.
...
static Symbol *domainSymbolForArray(Symbol *arr,
                                    ...
                                    AggregateType *&at)
{
  // Add arr to the aliasesAnalyzed map so we don't infinitely recurse
...
//##################################################################################
/*
    Insert a call to inspectorOn() after a loop. This is just a bit of
...
/*
    ...
    and "hasInvalidEnclosingStructure" flags. These will be checked on a per-function
    basis, where they control whether the optimized forall is executed. As such,
...
//##################################################################################
/*
    This function finds the locations that we want to insert calls to set/unset the
    ...
    It notes the locations that are furthest from the function and will record those
    ...
    traversal of the call graph by looking at a given FnSymbol's
...
/*
    ...
    The first call to this function is given call = B[expr] when considering a
    irregular access candidate. We want to ensure that "expr" does not contain any other
  ...
  for (int i = 1; i <= call->argList.length; i++) {
    if (CallExpr *callArg = toCallExpr(call->get(i))) {
      if (getCallBase(callArg)) {
        ...
        retval |= doesCallHaveNestedCallBase(callArg);
      }
    }
  }
  return retval;
} /* end of doesCallHaveNestedCallBase */
...
static void findCandidatesIWA(ForallStmt* forall)
{
  ...
  // Normally, we would iterate through the vector of CallExprs normally,
  // from first to last. The assumption (which I believe is accurate) is that
  // the CallExprs in the vector are in program order. We want to be able to allow
  // two aggregation candiates to be optimized in the loop, even if they deal with
  // the same array. Since both are doing aggregation, they're both order indepdent
  // and there's no data dependency. But if we go in program order when analyzing
  // candidates, it is difficult to determine this. So let's go through the vector
  // backwards. That way, when we look ahead of a given candidate, any accesses to
  // the same array that is also an aggregation candidate would have been processed
  // as such.
  for (auto it = callExprs.rbegin(); it != callExprs.rend(); ++it) {
    CallExpr *call = *it;
    // 1.) Ensure that call has a call-base, which means we
    //     have something like B[expr].
    if (getCallBase(call)) {
                                ...
                                ForallStmt *forall)
{
  ...
  */
  ...
  //
  // What we want to allow are accesses after the candidate which are aggregation candidates. Assuming
  // that this candidate and that one are valid, they are both order indepdent and can both be optimized.
  // Because findCandidatesIWA() iterates through the loop's CallExprs in reverse order, all we have to
  // do is check whether the given access is an aggregation candidate. If it is, we can skip it and move
  // on.
  //
...
/*
    ...
    well as those after the foreach loop (while still in the forall loop). However, we do
    allow accesses if they are an aggregation candidate.
...
/*
    ...
    We do allow for any statement that is part of an existing aggregation candidate, which
    we verify by look into the candidates vector. This means we can support multiple aggregations
    on the same data/base array.
    ...
    candidates -> current set of candidates in the forall
...
static void optimizeLoopIWA(ForallStmt *forall)
{
  ...
  //        PRIM_MAYBE_AGGR_CANDIDATE primitive
...
//##################################################################################
/*
    Creates the PRIM_MAYBE_AGGR_CANDIDATE primitive and replaces LHS's parent
    ...
    Return:
    the aggregated statement that is now the PRIM_MAYBE_AGGR_CANDIDATE primitive
*/
static CallExpr *createPrimitiveIWA(Expr *LHS,
                                    Expr *RHS,
                                    IrregWriteAggrOp op,
                                    ...
                                    int ID)
{
  ...
  CallExpr *repl = new CallExpr(PRIM_MAYBE_AGGR_CANDIDATE);
...
/*
    ...
    The main task here is to catch the PRIM_MAYBE_AGGR_CANDIDATE primitive
...
/*
    ...
    the PRIM_MAYBE_AGGR_CANDIDATE primitive.
