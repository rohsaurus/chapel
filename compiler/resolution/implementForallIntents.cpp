                                 ...
                                 SymbolMap& outer2shadow)
{
  ...
  for_vector(SymExpr, se, symExprs) {
    ...
    // if we find the flag FLAG_IE_PRIMITIVE_ARG, then we need
    // to continue/skip. This flag is added specifically to a static check
    // variable that is part of our MAYBE_IE_CANDIDATE primitive, which is inside
    ...
    // of the forall.
    if (sym->hasFlag(FLAG_IE_PRIMITIVE_ARG)) {
        continue;
    }
