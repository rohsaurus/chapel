...
# How to run via PBS: qsub -V -N run_tests -v INSPECTOR_ON=YES run_tests_pbs.sh
# the INSPECTOR_ON part is optional

#PBS -l select=2,place=scatter
#PBS -j oe
...
do
    DIFF=""
    DIFF=$(diff compiler_log/OPT_IS_VALID/${exe}.exe* latest_good_results/compiler_log_swan/OPT_IS_VALID/${exe}.exe*)
    if [ "$DIFF" != "" ]
    then
...
do
    DIFF=""
    DIFF=$(diff compiler_log/OPT_IS_NOT_VALID/${exe}.exe* latest_good_results/compiler_log_swan/OPT_IS_NOT_VALID/${exe}.exe*)
    if [ "$DIFF" != "" ]
    then
        printf "${RED} ${exe}: FAILED${NOCOLOR}\n"
    else
